...
def _paths() -> tuple[Path, Path, Path]:
    ...
    return (
        root,
        root / "docs" / "progress" / "phase3_m3_loss_repilot3_plan_20260713.json",
        root / "docs" / "progress" / "phase3_m3_loss_repilot3_batch_20260713.json",
    )
...
def test_repository_m3_repilot3_plan_is_authorized_for_pilot_execution_only() -> None:
    ...
    assert {assignment["seed"] for assignment in assignments.values()} == {957, 958}

    expected_mutations = {
        "m3_repilot3_r1_control_seed957": [],
        "m3_repilot3_r1_clarify_seed957": ["clarify_elderly_understanding_sales_only"],
        "m3_repilot3_r4_control_seed958": [],
        "m3_repilot3_r4_contradict_seed958": ["contradict_chat_approval_recorded"],
    }
    for run in batch.runs:
        assert str(run.run_root).startswith("runs/phase3_m3_repilot3_20260713/")
...
def test_repository_m3_repilot3_plan_records_v3_generation_boundary() -> None:
    _, plan_path, _ = _paths()
    plan = json.loads(plan_path.read_text(encoding="utf-8"))
    note = plan["measurement_boundary"]["generation_note"]
    assert "workflow_support_v3" in note
    assert "STR-01" in note
    assert "Never pooled" in note
    assert plan["execution_conditions"]["seeds"] == [957, 958]
