...
| 演算子 | 例 | 測るもの |
|---|---|---|
| clarify | AMB-02へ「70歳以上」通達（全配信 vs 索引非対称配信） | 明確化は分岐を減らすか。非対称配信ならstale-document効果 |
| contradict | AMB-04dへFAQ追記「チャット承認は記録すれば可」 | 緩い文書は後追い運用を正規化するか |
| version_skew | 旧zip(v1.0)の021等を担当者索引に残置 | 旧版参照者だけの別行動 |
| role_table_fix | CONTRA-01を1部署に統一 | 帰属矛盾の解消は誤宛先報告を減らすか |
| dangling_fill | STR-01の参照先スタブを実体化 or 放置 | 参照不在時の行動（諦め/同一視/照会） |
...
- **L1 構造（全ラン・機械）**: 承認者集中（ジニ係数）、代替承認連鎖、差戻し間隔異常、チャット言及→行為の時間相関（黙認シグナル）、検知漏れ=recorder事実−(L0∪登録済み既存ルール)検出。
...
指標は機会あたり発生率のみ（生カウント禁止）。config間比較はK seeds上のWilson区間＋対アンカー効果量。seed間ICC低のバケットは「モデル気まぐれ」棚へ隔離。バケットは探索的発見であり、**所見化には事前登録した対照ペアでの確認ラン再現を必須**とする（探索データと確認データを混ぜない）。
