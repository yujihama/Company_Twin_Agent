...
**損失事象オラクルv1** (`src/company_twin/loss_oracle.py`、CLI `loss-events`):
実験者側の潜在真実を判定材料に、損失事象そのものを機械判定する。
(a) `unapproved_completion`: 承認必須プローブ案件が承認ゼロで契約・交付に到達、
(b) `unconfirmed_vulnerable_sale`(候補水準): 理解脆弱性のある顧客の案件が
顧客対応記録ゼロで完了(v1は構造判定のみ——記録の意味的十分性は判定しない。
v2で判定器を較正してから導入)、(c) `abandonment_with_complaint`: 反復督促後の
顧客離脱(帰結v2)。既知の限界(誤った顧客IDでの記録は照合漏れ=過大方向)は
出力に明記。従来の検査器(旧版引用・根拠整合)は中間事象の指標として存続し、
主要評価は今後、損失事象の発生率と検知漏れ率に置く。
テスト: `tests/test_loss_oracle.py`。

**追記(2026-07-10・オーナー決定): 主対象をR1〜R4に限定。** レベル感の精査により、
旧R6(顧客放置による失注・苦情)は損失事象ではなく業務影響指標(機会損失はオペリスク
損失データの定義外)、旧R7(証跡不全)は事象でなく潜在エクスポージャと判定。研究が
ぼやけるため、主対象は単件発生が管理上意味を持つR1〜R4(法令違反級・内部管理違反級)
に絞る。オラクルv1は`unverified_completion`(R3)を追加し、離脱は
`business_impact_indicators`として損失事象と分離、全所見に級を刻印。
