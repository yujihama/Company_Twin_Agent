...
追加した。M3用の実campaign planは未封印、live M3は未実行であり、
...
M3へ適用する具体的な時間窓等はまだ封印していない。新しい世界内検知通知の追加と
readiness接続は本§の範囲外。テスト: `tests/test_loss_monitoring.py`。
...
**実行・承認境界.** 本§で完成するのは汎用集計器とsynthetic fixtureによる検証であり、
M3の実campaign plan、live run、結果はまだ存在しない。次のPRで対象mutation/probe、arm、
fresh seed/K、主要分母、pre-event、post-event window、R3最小opportunityとscope、plan外損失の扱い、
および直接発見統制を世界へ追加するかをオーナー判断のうえ封印する。直接発見通知の追加は
